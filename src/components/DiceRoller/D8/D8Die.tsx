...
export default function D8Die({ rolling, onResult, onRollEnd, onClick }: DieRendererProps) {
  const { rotation, result, faceNums, landed, spinCount } = useFlatSpin(8, rolling, onResult, onRollEnd);

  return (
    <CubeShell die={8} rolling={rolling} landed={landed} rotation={rotation} spinCount={spinCount} onClick={onClick}>
      <div className="dr__face dr__face--front">
        <FaceNumber value={result ?? '\u2014'} label="d8" />
      </div>
