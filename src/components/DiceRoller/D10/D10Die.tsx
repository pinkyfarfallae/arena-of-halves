...
export default function D10Die({ rolling, onResult, onRollEnd, onClick }: DieRendererProps) {
  const { rotation, result, faceNums, landed, spinCount } = useFlatSpin(10, rolling, onResult, onRollEnd);

  return (
    <CubeShell die={10} rolling={rolling} landed={landed} rotation={rotation} spinCount={spinCount} onClick={onClick}>
      <div className="dr__face dr__face--front">
        <FaceNumber value={result ?? '\u2014'} label="d10" />
      </div>
